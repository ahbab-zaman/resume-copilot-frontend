...
> **Project-specific addendum (AI Resume Job Pilot):** Before diagnosing, check whether the failure is actually a **cross-repo contract mismatch** — the frontend sending or expecting something different from what the backend's REST contract in `architecture.md` documents. This usually presents as Failure Mode 1 (a specific, isolated thing broken) but the root cause and the fix may live in the _other_ repo. Always check `architecture.md`'s REST API Contract table in both repos before diagnosing a broken request/response as a normal bug in this repo alone.
...
- Is this isolated to this repo, or could the other repo (frontend/backend) be involved?
```
...
- The problem is isolated — one component, one function, one route — or it is a contract mismatch between the two repos with a clear, identifiable difference (e.g. the backend renamed a field and the frontend still reads the old name)
...
**What it means:**
This is a normal bug. It has a root cause that can be found and fixed precisely. If it's a contract mismatch, the root cause lives in whichever repo last changed the shape without updating the other — confirm against `architecture.md` before fixing either side.
...
Read the relevant code. Do not read the entire codebase — only what is directly relevant to the problem. If it's a possible contract mismatch, read the relevant section of `architecture.md`'s REST API Contract table alongside the code on this side of the boundary.
...
Describe the fix that addresses the root cause. Not a workaround. Not a patch on top of broken code. If the fix needs to happen in the _other_ repo, say so explicitly rather than working around it on this side.

```
Fix: [what needs to change and why]
[If applicable: This fix belongs in the [frontend/backend] repo, not here.]
...
- Does the other repo need to know anything about this (e.g. a contract decision that's now uncertain)?
...
### Cross-repo notes
[Anything the other repo's session needs to know, if relevant]
```
