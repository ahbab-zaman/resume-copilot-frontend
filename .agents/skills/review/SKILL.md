...
- **State management correctness** _(frontend only)_ — is server data (resumes, analyses, applications, dashboard stats) read through a TanStack Query hook, not duplicated into Redux? Is Redux used only for genuine client-only, cross-page state? Is local `useState` used for anything that doesn't actually need to be global? Check against `code-standards.md`'s State Management section — this is one of the most common places AI-written code drifts into an anti-pattern (caching the same data in two places).
