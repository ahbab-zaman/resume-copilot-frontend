...
> **Project-specific addendum (AI Resume Job Pilot):** This skill is **frontend-repo only**. The backend (Express) has no UI components, so there is nothing to imprint there — `ui-registry.md`, `ui-rules.md`, and `ui-tokens.md` all live exclusively in the frontend repo's `context/` folder. Run this skill after building anything in `components/` (Sidebar, AtsScoreDashboard, KanbanBoard, QuestionCard, etc.) — never run it against backend code.
...
If no filepath was provided — identify which component files were most recently created or modified in this session. Look in `components/` (this project's UI lives under `components/ui/`, `components/layout/`, `components/landing/`, `components/copilot/`, `components/resumes/`, `components/applications/`, `components/interview/`, `components/settings/`) and read those files.
...
Read the component code. Extract only the classes and values that affect visual consistency across the interface. Not everything — only what makes components look like they belong together. Reject anything that isn't already a token defined in `ui-tokens.md` — if a class doesn't trace back to a `@theme` variable from that file, flag it as a violation rather than recording it as a new pattern.
...
At the start of any session that involves UI work, Claude reads `ui-registry.md` (and `ui-rules.md` / `ui-tokens.md`) before writing any component. When building a new card, it checks how existing cards were built. When building a new button, it checks what button patterns already exist. When building a new ATS score badge or Kanban card, it matches the exact classes already in use.
...
Find every component file in the project under `components/`. Read each one. Build a complete picture of what visual patterns are currently in use across the entire interface.
...
Recommendation: [which one to standardise on and why — should map to --radius-sm/md/lg/full from ui-tokens.md]
...
[List every hardcoded hex value, raw Tailwind color class (e.g. bg-blue-500), or
non-token value found — with the file and line where it appears]
These must be replaced with design system tokens from ui-tokens.md.

### Recommended baseline
[The correct pattern for each property —
based on what the majority already uses correctly
and what ui-tokens.md / ui-rules.md define]
```
