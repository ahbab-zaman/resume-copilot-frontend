...
> **Project-specific addendum (AI Resume Job Pilot):** This repo (frontend) keeps its **own** `memory.md` — the backend repo has a separate one. They are not merged or shared, since the two are different sessions/codebases. If a decision made here affects the other repo (e.g. a change to the REST contract, a new field better-auth's JWT now includes), say so explicitly under "Open questions" or "Next session starts with" so the developer remembers to carry it over manually. Never write `GOOGLE_CLIENT_SECRET`, `DATABASE_URL`, JWT signing keys, or any value from `.env.local` into `memory.md` — redact as `[REDACTED]` if one appears in conversation.
...
- For this project specifically: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `DATABASE_URL`, `NEXT_PUBLIC_BACKEND_URL` is fine to keep (not a secret) but never the keys above
...
**What was built** — specific files created or modified, features completed, components added. Be precise. Not "built the auth flow" — "created `lib/auth.ts`, `lib/auth-client.ts`, `app/api/auth/[...all]/route.ts`. Email/password + Google OAuth working end to end, JWT plugin enabled."

**Decisions made** — choices that would be hard to reverse or that future work depends on. Not implementation details — architectural choices. "Chose to fetch a fresh JWT per backend call rather than caching it across requests — tokens are short-lived, caching risked sending an expired one."

**Problems solved** — any issue that took time to figure out. So the next session does not solve the same problem twice. "better-auth's JWKS endpoint needs `NEXT_PUBLIC_APP_URL` set correctly in prod or the backend's `jose` verification fails silently with a generic 401."
...
**Open questions** — anything unresolved that the next session needs to address, including anything the _other_ repo's session needs to know about.
...
Write the memory to `memory.md` in **this repo's** root (frontend). This file always contains only the most recent session state for the frontend. It is not shared with the backend repo's `memory.md`.
...
```markdown
# Memory — [Feature or Session Name] (Frontend)
...
[Anything unresolved that needs addressing, including anything the backend repo's session needs to know]
```
...
[If relevant: Note — the backend repo may also need to know: [summary].]
```
...
Look for `memory.md` in this repo's root. If it does not exist, tell the developer:

```
No memory.md found in this repo.
...
- This repo's `context/` folder (`project-overview.md`, `architecture.md`, `build-plan.md`, `progress-tracker.md`, `code-standards.md`, `library-docs.md`, and for frontend also `ui-rules.md`, `ui-tokens.md`, `ui-registry.md`)
...
```
Memory restored. Here is where we are (frontend repo):
